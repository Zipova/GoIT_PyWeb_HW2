...
class AbsOutput(metaclass=ABCMeta):
    @abstractmethod
    def run(cli):
        pass


class ConsoleOutput(AbsOutput):
    @staticmethod
    def run(cli):
        print('It is Your Contact Book. You can enter "help" to get a list of commands, "close" - to exit.')
        while True:
            user_input = input("Please enter command: ")
            result = PhonebookInputParser().parse_user_input(user_input=user_input)
            if len(result) != 2:
                print(result)
                continue
            command, arguments = result
            command_function = getattr(cli, command.replace(" ", "_") + "_handler")
            try:
